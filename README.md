...
## 2.5 모듈 사용법 — 팀 통합 (정적분석 + 평판)

> sandbox / verifier는 다른 팀원이 개발합니다. 이 저장소에서 우리 팀이 책임지는
> 것은 **정적분석 모듈(`static_analyzers/`)** 과 **평판 조회 모듈(`reputation/`)**
> 두 개이며, 둘 다 **증거 제공자(evidence provider)** 입니다 — ALLOW/BLOCK을
> 직접 내리지 않고, 정규화된 결과를 verifier가 종합하도록 넘깁니다.

### 호출 방법 (두 모듈 공통 시그니처)

두 모듈 모두 동일한 4-인자(+선택)로 호출하고, 내부에서 `artifact_classifier`가
action의 targets를 타입별 노드(pypi/npm/repo/skill)로 분해해 타입별 분석기로
디스패치합니다.
...
# action: {"type": "command", "command": "...", ...}  (agent가 하려는 행위)
# context: {"cwd": "...", "task": "...", ...}
# targets: external_target_extractor.extract_external_targets(action, context, classification) 결과
# classification: {"external_env": True, "reasons": [...]}  (외부 소스일 때만 분석)

static_result = analyze_static(action, context, targets, classification, config=None)
reputation_result = analyze_reputation(action, context, targets, classification)
```

`external_env=False`(외부 소스 아님)이거나 분석 대상 코드가 없으면 둘 다
`status="skipped"`를 반환합니다. 노드 1개만 직접 분석하려면 저수준 진입점:
`static_analyzers.analyze_node(node, cfg)` / `reputation.lookup_node(node, timeout=10)`.

### 정적분석 출력 (verifier로 넘기는 형식)

```python
{
  "status": "success" | "skipped" | "unavailable",   # success=분석이 증거 산출(빈 findings 가능),
                                                       # unavailable=인프라 장애로 분석 불가
  "findings": [ {"rule_id", "severity", "path", "line", "message", "source"} ],
  "summary": "<one-line>",
  "scan_root": "/tmp/.../pkg",
  "rules": "p/security-audit",
  "per_artifact": [ {"artifact_type", "status", "findings", "summary", ...} ],  # 타입별 분해
}
```

- `severity` ∈ `CRITICAL | HIGH | MEDIUM | LOW` (verifier의 risk_level 어휘와 동일).
- `source` ∈ `semgrep | gitleaks | obfuscation-heuristic | npm-manifest-heuristic | chanever-skill | semgrep-meta`.
- **`status="success" + findings=[]`** 는 "알려진 악성 패턴 없음"일 뿐 "안전 증명"이 아님. timeout은 unavailable이 아니라 휴리스틱 결과를 담은 success. `static.scan-error` finding이 있으면 룰 로드 실패로 분석이 부분적이라는 신호.

### 평판 출력 (verifier로 넘기는 형식)

```python
{
  "status": "success" | "skipped" | "unavailable",
  "signals": [ {"source", "target_type", "target_name", "ecosystem", "status",
                "vuln_count", "severities", "known_bad_package", "known_bad_sources",
                "typosquat": {...}, "summary", ...} ],   # 노드/소스별 1개
  "summary": "<one-line>",
}
```

- 평판은 **출처 신뢰성**만 봅니다(작성자·배포처·이력·known-bad 목록 = DataDog/OSSF 인용·OSV·deps.dev·Scorecard). 콘텐츠 패턴 분석은 정적분석 몫 — 두 축은 겹치지 않게 분리돼 있습니다.

### Evidence Package 안에서의 위치

`shadow_sandbox_safeguard`가 두 결과를 `external_interaction_analysis` 아래에
**변환 없이 그대로** 넣어 Evidence Package(JSON)로 저장합니다:

```python
external_interaction_analysis = {
  "targets": [...],
  "static_analysis": <analyze_static 결과>,
  "reputation_analysis": <analyze_reputation 결과>,
}
```

verifier는 이 둘 + sandbox 실행 trace를 종합해 최종 verdict를 냅니다.

### 검증 상태 (우리 두 모듈)

- 정적분석: 전수 census 192 케이스 → 129 TP / 25 FN / 38 TN, **benign 0 FP(특이도 100%)**, recall 83.8%, 0 UNAVAIL. (`bench/static_analysis_reliability.py`)
- 평판: 라벨 패널 contingency → known-bad 판정이 출처 인용과 일치. (`bench/reputation_reliability.py`)
- 스코프: pypi/npm/repo/skill 4종. container_image는 검증 corpus 부재로 제외(detect는 하되 정적분석 skip).
